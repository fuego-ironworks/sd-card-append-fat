...
static int fat_ioctl_set_attributes(struct file *file, u32 __user *user_attr)
{
	...
	err = appendfat_setattr(file_mnt_idmap(file), file->f_path.dentry, &ia);
	if (err)
		goto out_unlock_inode;
...
static int fat_ioctl_fitrim(struct inode *inode, unsigned long arg)
{
	...
	err = appendfat_trim_fs(inode, &range);
	if (err < 0)
		return err;
...
long appendfat_generic_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
...
static int fat_file_release(struct inode *inode, struct file *filp)
{
	if ((filp->f_mode & FMODE_WRITE) &&
		...
		appendfat_flush_inodes(inode->i_sb, inode, NULL);
...
int appendfat_file_fsync(struct file *filp, loff_t start, loff_t end, int datasync)
{
...
const struct file_operations appendfat_file_operations = {
	.llseek		= generic_file_llseek,
	.read_iter	= generic_file_read_iter,
	.write_iter	= generic_file_write_iter,
	.mmap_prepare	= generic_file_mmap_prepare,
	.release	= fat_file_release,
	.unlocked_ioctl	= appendfat_generic_ioctl,
...
static int fat_free(struct inode *inode, int skip)
{
	...
	if (skip) {
		...
		ret = appendfat_get_cluster(inode, skip - 1, &fclus, &dclus);
		if (ret < 0)
			return ret;
		...
		fatent_init(&fatent);
		ret = appendfat_ent_read(inode, &fatent, dclus);
		if (ret == FAT_ENT_EOF) {
			...
			err = appendfat_ent_write(inode, &fatent, FAT_ENT_EOF, wait);
			if (err)
				ret = err;
		}
		fatent_brelse(&fatent);
	...
	return appendfat_free_clusters(inode, free_start);
}

void appendfat_truncate_blocks(struct inode *inode, loff_t offset)
{
	struct msdos_sb_info *sbi = MSDOS_SB(inode->i_sb);
...
EXPORT_SYMBOL_GPL(appendfat_fileattr_get);

int appendfat_getattr(struct mnt_idmap *idmap, const struct path *path,
...
EXPORT_SYMBOL_GPL(appendfat_getattr);
...
int appendfat_setattr(struct mnt_idmap *idmap, struct dentry *dentry,
		struct iattr *attr)
{
	struct msdos_sb_info *sbi = MSDOS_SB(dentry->d_sb);
	...
	if (attr->ia_valid & ATTR_SIZE) {
		error = appendfat_block_truncate_page(inode, attr->ia_size);
		if (error)
			goto out;
	 ...
	 * ourselves.  See appendfat_truncate_time for the c/mtime logic on fat.
	 */
	if (attr->ia_valid & ATTR_ATIME)
		appendfat_truncate_time(inode, &attr->ia_atime, FAT_UPDATE_ATIME);
	if (attr->ia_valid & ATTR_MTIME)
		appendfat_truncate_time(inode, &attr->ia_mtime, FAT_UPDATE_CMTIME);
...
EXPORT_SYMBOL_GPL(appendfat_setattr);

const struct inode_operations appendfat_file_inode_operations = {
